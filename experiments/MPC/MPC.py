...
def jax_osqp_nonfixedpt(K_max, P, q, A, l_rest, u_rest, xinit_samp, x0, z0, y0, rho, sigma):
    ...
    def body_fun(k, val):
        ...
        # x[k] = VP.implicit_linear_step(lhs_mat.todense(), sigma * x[k-1] + rho * A.T @ (z[k-1] - y[k-1]),
...
def osqp_run(cfg, qc, P, q, A, l, u):
    ...
    for k in range(1, K+1):
        ...
        # w[k] = VP.saturated_linear_param_step(v[k-1], l_param, u_param, relax_binary_vars=False, equality_ranges=[(0, eq_idx_max)])
...
def osqp_nonfixedpt_run(cfg, qc, P, q, A, l, u):
    ...
    for k in range(1, K+1):
        ...
        x[k] = VP.implicit_linear_step(lhs_mat.todense(), sigma * x[k-1] + rho * A.T @ (z[k-1] - y[k-1]),
            lhs_mat_factorization=lhs_factored)
        # x[k] = lhs_mat_inv @ (sigma * x[k-1] + rho * A.T @ (z[k-1] - y[k-1]))
        ...
        plot_data(cfg, cfg.T, Deltas, max_sample_resids, rel_LP_sols, max_sample_resids, Delta_gaps, num_bin_vars, times)
